...
The `precheck` bash node has already confirmed that `templates/shorts/anthropic/`
...
the Anthropic dark-stage aesthetic (warm orange + cream + purple, near-black
canvas). Narration must clock in at approximately
...
**You MUST NOT modify `templates/shorts/anthropic/` itself.** Only edit the
...
[`.claude/skills/diy-yt-creator/new-anthropic-short.md`](../../.claude/skills/diy-yt-creator/new-anthropic-short.md).
...
`.claude/skills/diy-yt-creator/new-anthropic-short.md` in order. The exact
...
- Step 2 (copy template) — run:
  ```bash
  cp -r templates/shorts/anthropic "videos/$parse-input.output.slug"
  ```
  PowerShell: `Copy-Item -Recurse templates/shorts/anthropic videos/$parse-input.output.slug`
...
- Step 8 (edit `index.html`) — follow the 14 sub-steps in the playbook
  ...
  - The orange→cream gradient text-fill appears on AT MOST one element
    per video. The hero slam carries it; subsequent slams use solid
    accents.
  - No cyan or magenta anywhere — those belong to the archon template.
...
- [ ] `videos/$parse-input.output.slug/index.html` has been edited:
      every placeholder replaced with real content; transition timestamps,
      slam-shake offsets, and gradient drift anchor all recomputed from the
      transcript; `<audio id="narration">` wired.
...
`$ARTIFACTS_DIR/create-anthropic-short-summary.md`. Include:

```markdown
# create-anthropic-short — $parse-input.output.slug
